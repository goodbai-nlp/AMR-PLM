...
class SummarizationModule(BaseTransformer):
    ...
    def validation_epoch_end(self, outputs, prefix="val") -> Dict:
        ...
        for idx, tokens_same_source in enumerate(preds):
            ...
            lin_sentences.append(str(idx) + " " + self.tokenizer.decode(tokens_same_source).strip())
        json.dump(
            lin_sentences,
    ...
    def add_model_specific_args(parser, root_dir):
        ...
        parser.add_argument("--hyparams_path", type=str, default=None, required=False)
        parser.add_argument(
            "--val_metric",
            type=str,
            default=None,
...
def main(args, model=None) -> SummarizationModule:
    ...
    checkpoints = list(
        sorted(glob.glob(os.path.join(args.checkpoint, "*.ckpt"), recursive=True))
    )
    model = AMRparsingModule.load_from_checkpoint(
        checkpoint_path=checkpoints[0],
        hparams_file=args.hyparams_path,
        map_location=None,
    )
