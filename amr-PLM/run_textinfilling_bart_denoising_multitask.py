...
def train(
    args, train_dataset, eval_dataset, collate_fn, model: PreTrainedModel, tokenizer: PreTrainedTokenizer, config
) -> Tuple[int, float]:
    """ Train the model """
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if args.mlm_amr:
                ...
                if step == 0 and epoch == 0:
                    save_dummy_batch2(args, masked_input, dec_input, labels, tokenizer, prefix='Mamr2amr')
            ...
            if args.mlm_text:
                ...
                dec_input = dec_input.to(args.device)
                if step == 0 and epoch == 0:
                    save_dummy_batch2(args, masked_input, dec_input, labels, tokenizer, prefix='Mtext2text')
                outputs = model(input_ids=masked_input, attention_mask=attention_mask, decoder_input_ids=dec_input, labels=labels)
            ...
            if args.mlm_text_plus_amr:
                if step % args.joint_train_interval == 0:
                    ...
                    masked_input, attention_mask, dec_input, labels = get_partial_textinf_joint_inputs(batch, tokenizer, args, inp='text', mlm_prob=mlm_prob)
                    masked_input = masked_input.to(args.device)
                    labels = labels.to(args.device)
                    dec_input = dec_input.to(args.device)
                    if step == 0 and epoch == 0:
                        save_dummy_batch2(args, masked_input, dec_input, labels, tokenizer, prefix='val_MtextAmr2text')
                    outputs = model(input_ids=masked_input, attention_mask=attention_mask, decoder_input_ids=dec_input, labels=labels)
            ...
            if args.mlm_amr_plus_text:
                if step % args.joint_train_interval == 0:
                    mlm_prob = 0.1 + global_step/args.max_steps * 0.75
                    masked_input, attention_mask, dec_input, labels = get_partial_textinf_joint_inputs(batch, tokenizer, args, inp='amr', mlm_prob=mlm_prob)
                    masked_input = masked_input.to(args.device)
                    labels = labels.to(args.device)
                    dec_input = dec_input.to(args.device)
                    if step == 0 and epoch == 0:
                        save_dummy_batch2(args, masked_input, dec_input, labels, tokenizer, prefix='val_TextMamr2amr')
            ...
            if args.mlm_joint_to_text:
                mlm_prob = 0.35
                masked_input, attention_mask, dec_input, labels = get_full_textinf_joint_inputs_partial_output(batch, tokenizer, args, inp='text', mlm_prob=mlm_prob)
                masked_input = masked_input.to(args.device)
                labels = labels.to(args.device)
                dec_input = dec_input.to(args.device)
                if step == 0 and epoch == 0:
                    save_dummy_batch2(args, masked_input, dec_input, labels, tokenizer, prefix='val_MtextMamr2text')
                outputs = model(input_ids=masked_input, attention_mask=attention_mask, decoder_input_ids=dec_input, labels=labels)
            ...
            if args.mlm_joint_to_amr:
                ...
                dec_input = dec_input.to(args.device)
                if step == 0 and epoch == 0:
                    save_dummy_batch2(args, masked_input, dec_input, labels, tokenizer, prefix='val_MtextMamr2amr')
    ...
    for batch in pbar:
        ...
        with torch.no_grad():
            ...
            if args.mlm_text_plus_amr:
                ...
                masked_input, attention_mask, dec_input, labels = get_partial_textinf_joint_inputs(batch, tokenizer, args, inp='text')
                masked_input = masked_input.to(args.device)
                labels = labels.to(args.device)
                dec_input = dec_input.to(args.device)
                outputs = model(input_ids=masked_input, attention_mask=attention_mask, decoder_input_ids=dec_input, labels=labels)
            ...
            if args.mlm_amr_plus_text:
                ...
                masked_input, attention_mask, dec_input, labels = get_partial_textinf_joint_inputs(batch, tokenizer, args, inp='amr')
                masked_input = masked_input.to(args.device)
                labels = labels.to(args.device)
                dec_input = dec_input.to(args.device)
                outputs = model(input_ids=masked_input, attention_mask=attention_mask, decoder_input_ids=dec_input, labels=labels)
...
def main():
    ...
    AMRDataset = AMRDataSetFast(
        ...
        max_tgt_length=args.block_size,
    )
    AMRDataset.setup()
    ...
    if args.do_train:
        ...
        global_step, tr_loss = train(args, train_dataset, dev_dataset, seq2seq_collate_fn, model, tokenizer, config)
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            model.to(args.device)
            result = evaluate(args, dev_dataset, seq2seq_collate_fn, model, tokenizer, config=config, prefix=prefix)
