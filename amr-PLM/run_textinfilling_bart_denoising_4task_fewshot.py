...
from model_utils import get_mlm_inputs, get_mlm_inputs_short, get_mlm_joint_inputs_full, get_mlm_joint_inputs_short, get_text_infilling_inputs, get_textinf_joint_inputs_full
...
def train(
    ...
    train_sampler_few = (
        RandomSampler(train_dataset_few) if args.local_rank == -1 else DistributedSampler(train_dataset_few)
    )
    train_dataloader = DataLoader(
        train_dataset, sampler=train_sampler, batch_size=args.train_batch_size, collate_fn=collate_fn, num_workers=4
    )

    train_dataloader_few = DataLoader(
        train_dataset_few, sampler=train_sampler_few, batch_size=args.n_gpu, collate_fn=collate_fn, num_workers=4
    )
    ...
    logger.info("  Num few-shot examples = %d", len(train_dataset_few))
    ...
    for epoch in train_iterator:
        epoch_iterator = tqdm(
            zip(train_dataloader, cycle(train_dataloader_few)), desc="Iteration", disable=args.local_rank not in [-1, 0]
        )

        if args.local_rank != -1:
            train_sampler.set_epoch(epoch)

        for step, (batch, batch_few) in enumerate(epoch_iterator):
        #for step, (batch, batch_few) in enumerate(zip(train_dataloader, cycle(train_dataloader_few))):
...
def main():
    ...
    AMRDataset_few = AMRDataSetFast(
        tokenizer=tokenizer,
        train_file=args.train_file_few,
        validation_file=args.val_file_few,
        test_file=args.test_file_few,
        pad_to_max_length=False,
        max_src_length=args.block_size,
        max_tgt_length=args.block_size,
    )
    AMRDataset_few.setup()

    # Dummy Test
    train_dataset = AMRDataset.train_dataset
    dev_dataset = AMRDataset.valid_dataset
    train_dataset_few = AMRDataset_few.train_dataset
    ...
    if args.do_train:
        ...
        global_step, tr_loss = train(args, train_dataset, train_dataset_few, dev_dataset, seq2seq_collate_fn, model, tokenizer, config)
